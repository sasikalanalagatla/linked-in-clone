...
public interface ConnectionRequestRepository extends JpaRepository<ConnectionRequest, Long> {
    ...
    @Query("SELECT cr FROM ConnectionRequest cr WHERE ((cr.sender = :user1 AND cr.receiver = :user2) OR (cr.sender = :user2 AND cr.receiver = :user1)) AND cr.status = 'PENDING'")
    Optional<ConnectionRequest> findPendingConnectionBetweenUsers(@Param("user1") User user1, @Param("user2") User user2);

    @Query("SELECT cr FROM ConnectionRequest cr WHERE ((cr.sender = :user1 AND cr.receiver = :user2) OR (cr.sender = :user2 AND cr.receiver = :user1)) AND cr.status = 'ACCEPTED'")
    Optional<ConnectionRequest> findAcceptedConnectionBetweenUsers(@Param("user1") User user1, @Param("user2") User user2);
